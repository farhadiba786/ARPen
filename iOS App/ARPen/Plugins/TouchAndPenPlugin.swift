    ...
    class TouchAndPenPlugin: Plugin,UserStudyRecordPluginProtocol {

    //reference to userStudyRecordManager to add new records
    var recordManager: UserStudyRecordManager!
    var pluginImage: UIImage? = UIImage.init(named: "TouchAndPen")
    ...
    func didUpdateFrame(scene: PenScene, buttons: [Button : Bool]){
        ...
        if(selected == true){
            if pressed{
                ...
                let scaleFactor = abs(penPosition.y) * 2
