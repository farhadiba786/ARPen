...
class PinchScaling: Plugin, UserStudyRecordPluginProtocol {
    ...
    func didUpdateFrame(scene: PenScene, buttons: [Button : Bool]) {
      ...
      guard let box = scene.drawingNode.childNode(withName: "currentBoundingBox", recursively: false) else{
        ...
        if(confirmed && selected){
            ...
            "finalWidthExact" : "\(finalWidth)",
            "finalHeightExact" : "\(finalHeight)",
            "finalLengthExact" : "\(finalLength)",
            ...
            print("finalWidthExact :", finalWidth)
            print("finalHeightExact: ", finalHeight)
            print("finalLengthExact: ", finalLength)
            print("finalLengthExact: ", finalLength)
    ...
    func reset(){
        ...
        /*//compute random width/height/length users should scale the object to
            let randomWidth = String(format: "%.1f",Float.random(in: 3...15)*100)
            let randomHeight = String(format: "%.1f",Float.random(in: 8...25)*100)
            let randomLength = String(format: "%.1f",Float.random(in: 3...12)*100)
            let headingLabel = UILabel(frame: currentView!.frame)
            headingLabel.font = UIFont.init(name: "Helvetica", size: 14)
            //Vary between width/ height/length
            let randomTarget = Float.random(in: 1...3)
            if randomTarget == 1{
                DispatchQueue.main.async {
                    self.headingLabel.text = "Scale width to: \(randomWidth)"
                }
            }
            if randomTarget == 2{
                DispatchQueue.main.async {
                    self.headingLabel.text = "Scale height to: \(randomHeight)"
                }
            }
            if randomTarget == 3{
                DispatchQueue.main.async {
                    self.headingLabel.text = "Scale length to: \(randomLength)"
                }
            }*/
    ...
    @objc func handleTap(_ sender: UITapGestureRecognizer){
        ...
        widthIncmStr = String(format: "%.2f",updatedWidth*100)
        heightIncmStr = String(format: "%.2f",updatedHeight*100)
        lengthIncmStr = String(format: "%.2f",updatedLength*100)
