    ...
    class ARPenScalingPlugin: Plugin,UserStudyRecordPluginProtocol {

    //reference to userStudyRecordManager to add new records
    var recordManager: UserStudyRecordManager!
    var pluginImage: UIImage? = UIImage.init(named: "ARPenScaling")
    var pluginIdentifier: String = "ARPenScaling"
    ...
    func didUpdateFrame(scene: PenScene, buttons: [Button : Bool]){
        ...
        if pressed{
            ...
            if selected == true {
                if (abs(updatedPenPosition.y)>abs(updatedPenPosition.x)){
                    let scaleY = (abs(updatedPenPosition.y) * 2)
                    print("scale: \(scaleY)")
                    box.scale = SCNVector3(x: scaleY, y: scaleY, z: scaleY)
                    
                }
                else{
                    let scaleX = (abs(updatedPenPosition.x) * 2)
                    print("scale: \(scaleX)")
                    box.scale = SCNVector3(x: scaleX, y: scaleX, z: scaleX)
